...
while (<EXP>){
	...
	if ($LineNr == 0){
		...
		### Check genome ###
		print "$PREFIX\/genomes/$Genome\/$Genome\.fa\n";		
...
sub CopyFiles {
	...
	(my $FastqDir, my $SampleNumber, my $Outputdir) = @_;
	 ...
	 while (my $FastQGZFile = readdir(ORIGINAL_DATA_DIR)){
		...
		## EDIT: we're using sample names instead of _S<sample number>	
		# if ($FastQGZFile =~ m/.*_S$SampleNumber\_.*fastq\.gz$/i){
		if ($FastQGZFile =~ m/$SampleNumber\_.*fastq\.gz$/i){
...
sub CollectCutSiteReadsAndProcess {
	...
	### MarkDuplicates ###
	system 	("$SAMTOOLSBINDIRECTORY/samtools index $Outputdir\/5_CutSiteReads/$CutAmBasis\.sorted.bam");
	system 	("/opt/Software/Python-3.6.5/bin/umi_tools dedup -I $Outputdir\/5_CutSiteReads/$CutAmBasis\.sorted.bam --paired --output-stats=deduplicated -S $Outputdir\/5_CutSiteReads/$CutAmBasis\.sorted.remdup.bam");
	#system 	("$JAVA_INSTALLATION -Xmx4g -jar picard.jar MarkDuplicates I=$Outputdir\/5_CutSiteReads/$CutAmBasis\.sorted.bam O=$Outputdir\/5_CutSiteReads/$CutAmBasis\.sorted.remdup.bam M=$Outputdir\/5_CutSiteReads/$CutAmBasis\.sorted.remdup.metrics REMOVE_DUPLICATES=false 2>> $PicardLogFile");
...
#			my $RepairEfficiency	= $Repairs/$Indels;
	...
	print VAR "Chromosome\tPositions\tType\tLength\tFlankingSequence (+ strand)\tAbsoluteFrequency\tRelativeFrequency\tNrOfReads\tFirstContainingReadName\n";
			...
			foreach my $Positions (sort keys %{$InDelsRef->{$CutSite}{$Chr}}){
				foreach my $Type (sort keys %{$InDelsRef->{$CutSite}{$Chr}{$Positions}}){
					foreach my $Length (sort keys %{$InDelsRef->{$CutSite}{$Chr}{$Positions}{$Type}}){
					    ...
					    my $read_pair = (sort keys %{$InDelsRef->{$CutSite}{$Chr}{$Positions}{$Type}{$Length}})[0];
					    print VAR "$Chr\t$Positions\t$Type\t$Length\t$Sequence\t$AbsoluteFrequencyInNoConflictReads\t$RelativeFrequencyInNoConflictFragments\t$NoConflictNrOfReads\t$read_pair\n";
					}
				}
			}
		}
